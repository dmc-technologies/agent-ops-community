...
- Decision: permit one Prime-only typed transition from `.agentops/gstack-prime-manifest.json` into the shared public-skill ownership manifest. Declare it only for the public Prime gstack plan when no shared manifest exists. Under the existing target lock, require the canonical public gstack repository, exact legacy schema, owner, pinned ref, canonical complete planned path set, current file fingerprints, single-link regular files, and planned modes. Retain no-follow descriptor authority over every accepted file and the retired manifest through each backup move. On a concurrent identity, content, or mode change, preserve the concurrent entry, roll back earlier operations, and abort before shared ownership publication. Treat the legacy manifest and its files as rollback-backed prior state; never prepublish a synthetic shared manifest.
