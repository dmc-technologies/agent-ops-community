...
- Decision: permit one Prime-only typed transition from `.agentops/gstack-prime-manifest.json` into the shared public-skill ownership manifest. Declare it only for the public Prime gstack plan when no shared manifest exists. Under the existing target lock, require the exact legacy schema, owner, pinned source descriptor and ref, canonical complete planned path set, current file fingerprints, single-link regular files, and planned modes. Treat the legacy manifest and its files as rollback-backed prior state; never prepublish a synthetic shared manifest.
