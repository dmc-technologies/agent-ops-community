...
- Current handoff: public pull request 13 preserves safety clauses, nested source and STOP gates, executable release drift detection, and provider-aware routing. Its latest Review Gate found that land-and-deploy combined GitLab with unknown providers and that retained downstream workflows relied on release artifacts no concrete Prime workflow prepared. The correction now stops unknown providers without provider commands, requires an explicit repository-owned GitLab merge/deploy procedure, defines the Prime release-preparation sequence, makes document-release prepare and verify its artifacts, and makes land-and-deploy execute a fail-closed artifact gate before merge.
- Verification: Ruff, 103 tests, the source-tree harness check, and whitespace validation pass. A real pinned update retained 31 gstack and 14 Superpowers skills with split provider stops, pinned version allocation, review provenance, VERSION/package/CHANGELOG/title checks, nested plan-review gates, and no excluded command routes or malformed fallback markers.
