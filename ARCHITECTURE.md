...
Prime gstack has one narrow migration from its retired owner-only manifest into shared deployment ownership. The installed public provider declares the exact Prime target, public channel, canonical public gstack repository, pinned ref, and complete planned destination set. Under the retained target lock, the transaction accepts the legacy manifest only when its strict schema, owner, ref, canonical gstack-only paths, current file fingerprints, link counts, and planned modes all match. Each accepted file and the retired manifest remain bound to a no-follow descriptor through their backup moves. If a non-cooperating process changes an entry after validation, adoption aborts, restores that concurrent entry, rolls back earlier replacements, and publishes no shared manifest. The legacy manifest and every owned file otherwise become ordinary rollback-backed prior state in the same transaction; unrelated files remain untouched, and rollback or crash recovery restores the exact legacy files and manifest.
