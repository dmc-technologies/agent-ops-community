...
Prime gstack has one narrow migration from its retired owner-only manifest into shared deployment ownership. The installed public provider declares the exact Prime target, public channel, canonical public gstack repository, pinned ref, and complete planned destination set. Under the retained target lock, the transaction accepts the legacy manifest only when its strict schema, owner, ref, canonical gstack-only paths, current file fingerprints, link counts, and planned modes all match. Before the first replacement, every accepted file and the retired manifest become bound to retained no-follow descriptors. If a non-cooperating process replaces, edits, changes the mode of, or deletes an entry after validation, the transaction journals that exact concurrent state before preserving it, rolls back earlier replacements, and publishes no shared manifest. Recovery resumes the same journaled sequence after a process exit and does not mark rollback complete until the concurrent replacement or deletion is restored. The legacy manifest and every owned file otherwise become ordinary rollback-backed prior state in the same transaction; unrelated files remain untouched, and rollback or crash recovery restores the exact legacy files and manifest.
