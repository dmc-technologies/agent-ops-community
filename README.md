...
`agentops deployment status --target codex-preview` reads the descriptor-confined preview manifest and owned target files without a managed fetch or receipt. It reports `preview` with the exact 64-hex fingerprint, `modified` when an owned file, mode, or directory differs, and `failed` when preview evidence is invalid.
