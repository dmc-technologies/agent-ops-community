...
Profiles created by the earlier Prime gstack installer can also move into the shared deployment transaction once. The transaction requires the canonical public gstack repository, exact legacy owner, pinned source ref, complete gstack-only path set, file fingerprints, and planned modes while the target lock is retained. It opens and retains no-follow authority over every accepted file and the retired manifest before any replacement begins. Every accepted legacy path is backup-backed even when its bytes already equal the current plan, and every move has exact pre-move authority in the transaction record. If the moved backup differs because another process changed the path after classification, recovery journals that safe backup as the concurrent entry before no-replace restoration. Adoption aborts, preserves that changed state, rolls back earlier replacements, and publishes no shared manifest; another live entry is never overwritten. The transaction removes the retired owner-only manifest only as a rollback-backed operation. Modified, missing, extra, wrong-owner, wrong-source, wrong-ref, or unsafe evidence stops the refresh without replacing the legacy files, and a later failure or crash restores the exact prior files and manifest.
