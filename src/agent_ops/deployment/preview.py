...
class PreviewEngine:
    ...
    def preview(
        ...
        for provider in supported:
            paths = provider.source_closure(provisional, target, selection)
            ...
            declared[provider.provider_id] = paths
            all_declared.update(paths)
        captured = _capture_tracked_closure(checkout, tuple(all_declared))
        fingerprint = _closure_fingerprint(captured)

        plans: list[ProviderPlan] = []
        for provider in supported:
            provider_entries = _provider_entries(
                captured, declared[provider.provider_id]
            )
...
def _tracked_entries(checkout: Path) -> dict[str, str]:
    ...
    for record in listing.split("\0"):
        ...
        mode, _object_id, stage = descriptor.split(" ", 2)
        if stage != "0":
...
def _capture_tracked_closure(
    checkout: Path, declared: tuple[Path, ...]
    ...
    selected_files = {
        path
        for path in tracked
        if any(path == root or path.startswith(f"{root}/") for root in raw_declared)
    }
    if any(
        root not in tracked and not any(path.startswith(f"{root}/") for path in tracked)
        for root in raw_declared
    ):
        raise ValueError("every referenced preview resource must be Git-tracked")
    if any(tracked[path] == "120000" for path in selected_files):
    ...
    try:
        ...
        captured = tuple(_capture_entry(root_fd, path) for path in paths)
        if _identity(os.stat(checkout, follow_symlinks=False)) != root_identity:
            raise RuntimeError("selected source checkout changed during capture")
        return captured
    finally:
        os.close(root_fd)


def _capture_entry(root_fd: int, path: Path) -> _CapturedEntry:
    ...
    try:
        ...
        permission_mode = stat.S_IMODE(before.st_mode)
        ...
        if stat.S_ISDIR(before.st_mode):
            if permission_mode & 0o700 != 0o700:
                raise ValueError("selected source directory mode must grant owner access")
            kind = "directory"
            content = None
        elif stat.S_ISREG(before.st_mode):
            if permission_mode & 0o400 == 0:
                raise ValueError("selected source file mode must be owner-readable")
            kind = "file"
            chunks: list[bytes] = []
            while chunk := os.read(current, 64 * 1024):
                chunks.append(chunk)
        ...
        return _CapturedEntry(path, kind, permission_mode, content)
    finally:
        os.close(current)
