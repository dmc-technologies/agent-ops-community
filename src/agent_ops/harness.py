...
- When repository instructions declare an integration branch, routine feature
  pull requests hand off through exact pull-request and tracker evidence; the
  integration controller owns repository progress and stable-branch landing
  state.
...
7. Determine the target branch and whether repository instructions declare an
   integration branch, a routine feature pull request, integration-controller
   work, or stable-branch landing.
8. Run the fastest relevant verification command before broad edits when feasible.
9. Continue from `.agentops/harness/PROGRESS.md` "Next Actions" when that file
   is owned by the current branch role; otherwise continue from exact pull
   request and tracker evidence.

## Clock Out

1. If no integration branch is declared, update
   `.agentops/harness/PROGRESS.md` with current state, verification, blockers,
   and next actions.
2. If repository instructions declare an integration branch, hand off a
   routine feature pull request through exact pull request and tracker evidence;
   do not replace `.agentops/harness/PROGRESS.md`.
3. The integration controller or stable-branch landing updates
   `.agentops/harness/PROGRESS.md` when repository instructions assign that
   role.
